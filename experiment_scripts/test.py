...
p.add_argument('--logging_root', type=str,
               default='./logs', help='root for logging')
...
p.add_argument('--num_hl', type=int, default=3, required=False,
               help='The number of hidden layers')
p.add_argument('--num_nl', type=int, default=512, required=False,
               help='Number of neurons per hidden layer.')
p.add_argument('--pretrain_iters', type=int, default=5000,
...
p.add_argument('--counter_end', type=int, default=90000, required=False,
               help='Defines the linear step for curriculum training starting from the initial time')
p.add_argument('--num_src_samples', type=int, default=10000,
               required=False, help='Number of source samples at each time step')

p.add_argument('--omega_max', type=float, default=1.1,
               required=False, help='Gimbal rate of the engines')
p.add_argument('--minWith', type=str, default='zero', required=False,
               choices=['none', 'zero', 'target'], help='BRS vs BRT computation')
...
p.add_argument('--seed', type=int, default=0, required=False,
               help='Seed for the simulation.')

p.add_argument('--checkpoint_path', default=None,
               help='Checkpoint to trained model.')
p.add_argument('--checkpoint_toload', type=int, default=0,
               help='Checkpoint from which to restart the training.')
opt = p.parse_args()
...
l = torch.tensor(
    np.array([[engine_l, engine_circ_r, 0.0],
              [engine_l, -engine_circ_r /
               np.sqrt(2.0), -engine_circ_r/np.sqrt(2.0)],
              [engine_l, -engine_circ_r/np.sqrt(2.0), engine_circ_r/np.sqrt(2.0)]]), dtype=torch.float32)

numpoints = 65000
dataset = dataio.ReachabilityAttitudeSource(numpoints=2,
...
def val_fn(model, ckpt_dir, epoch):
  ...
  for i in range(num_times):
    ...
    for j in range(num_thetas):
      ...
      coords = torch.cat((time_coords, qx, mgrid_coords, omega, theta_coords, theta_other), dim=1)
      print(coords.shape)
...
#       ckpt_dir, 'BRS_validation_plot_epoch_%04d.png' % epoch))
