...
def get_mgrid_attitude(sidelen, dim=2):
    '''Generates a flattened grid of (x,y,...) coordinates in a range of -1 to 1.'''
    if isinstance(sidelen, int):
        sidelen = dim * (sidelen,)
...
class ReachabilityAir3DSource(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # uniformly sample domain and include coordinates where source is non-zero 
        coords = torch.zeros(self.numpoints, self.num_states).uniform_(-1, 1)
        # coords = torch.zeros(self.numpoints, self.num_states)
        # coords[:, 1] = -0.51
        # print(self.collisionR)
...
class ReachabilityAttitudeSource(Dataset):
    def __init__(self, numpoints,
                 target_set, J, J_inv, l, M=3, thrust=1.0, omega_max=1.1, ang_rate_max=2.0*np.pi, theta_max=0.25,
                 pretrain=False, tMin=0.0, tMax=0.5, counter_start=0, counter_end=100e3,
        ...
        self.thrust = thrust
        self.omega_max = omega_max
        self.ang_rate_max = ang_rate_max
        self.theta_max = theta_max

        self.target_set = target_set
        self.target_set[6:] = self.target_set[6:] / self.theta_max
        self.target_set[3:6] = self.target_set[3:6] / self.ang_rate_max

        self.J = J.cuda()
        self.J_inv = J_inv.cuda()
        self.l = l.cuda()
        self.M = M
    ...
    def __getitem__(self, idx):
        ...
        # uniformly sample domain and include coordinates where source is non-zero
        coords = torch.zeros(self.numpoints, self.num_states).uniform_(-1, 1)
        # coords = torch.ones(self.numpoints, self.num_states)
        # coords[:, 1] = -0.06
        # coords[:, 3:5] = -0.06
        ...
        # set up the initial value function
        boundary_values = torch.max(torch.abs(coords[:, 1:]) - self.target_set[None, :], dim=1, keepdim=True)[0]
